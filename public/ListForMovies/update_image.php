...
$check_sql = "SELECT id FROM movies WHERE id = ? AND user_id = ?";
...
if ($check_result->num_rows === 0) {
    die("Error: Movie not found or you do not have permission to update it.");
}
$check_stmt->close();
...
if ($use_default_image) {
    ...
    if (!empty($_FILES['img_file']['name'])) {
        ...
        $new_file_name = uniqid("movie_img_", true) . "." . $imageFileType;
...
$update_sql = "UPDATE movies SET img_url = ?, img_file_path = ? WHERE id = ? AND user_id = ?";
$update_stmt = $mysqli->prepare($update_sql);

if ($update_stmt) {
    $update_stmt->bind_param("ssii", $img_url, $img_file_path, $movie_id, $user_id);
