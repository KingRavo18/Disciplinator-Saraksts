...
$check_sql = "SELECT id FROM games WHERE id = ? AND user_id = ?";
...
if ($check_result->num_rows === 0) {
    die("Error: Game not found or you do not have permission to update it.");
}
$check_stmt->close();
...
$update_sql = "UPDATE games SET img_url = ?, img_file_path = ? WHERE id = ? AND user_id = ?";
$update_stmt = $mysqli->prepare($update_sql);

if ($update_stmt) {
    $update_stmt->bind_param("ssii", $img_url, $img_file_path, $game_id, $user_id);
