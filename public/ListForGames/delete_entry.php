...
if (!isset($_SESSION['user_id'])) {
    die("Unauthorized: You must be logged in to delete a game.");
}
...
if (!isset($_POST['game_id']) || !filter_var($_POST['game_id'], FILTER_VALIDATE_INT)) {
    die("Invalid game ID.");
}
...
$check_sql = "SELECT img_file_path FROM games WHERE id = ? AND user_id = ?";
...
if ($result->num_rows === 0) {
    die("Error: You do not have permission to delete this game.");
}
...
if ($stmt) {
    ...
    if ($stmt->execute()) {
        ...
        error_log("Delete Game Error: " . $stmt->error); 
