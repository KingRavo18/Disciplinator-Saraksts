...
if (isset($_SESSION['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $user_id = $_SESSION['id'];
        // First, check if the task is already failed or completed
        $sql = "SELECT is_failed, is_completed FROM tasks WHERE id = ? AND user_id = ?";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param('ii', $task_id, $user_id);
        $stmt->execute();
        $stmt->bind_result($is_failed, $is_completed);
        $stmt->fetch();
        $stmt->close();
        
        // If the task is not failed and not completed, update it and decrement the user's points
        if (!$is_failed && !$is_completed) {
            // Update the task as failed
            $sql_update_task = "UPDATE tasks SET is_failed = 1 WHERE id = ? AND user_id = ?";
            ...
            $sql_update_points = "UPDATE users SET points = GREATEST(COALESCE(points, 0) - 1, 0) WHERE id = ?";
        ...
        // Redirect back to the main page after marking the task as failed
